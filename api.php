...
function http_get($url, $headers = []) {
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_TIMEOUT        => 90,  // 👈 Modifié : 90 secondes au lieu de 10
...
if ($action === 'get_notifications_list') {
    ...
    if ($radarr) {
        ...
        $h = arr_get($radarr, '/api/v3/history?pageSize=60&sortKey=date&sortDirection=descending&eventType=3');
        $baseUrl = rtrim($radarr['url'], '/');
    ...
    if ($sonarr) {
        // 🌟 MODIFIÉ : pageSize=60 au lieu de 30
        $h = arr_get($sonarr, '/api/v3/history?pageSize=60&sortKey=date&sortDirection=descending&eventType=3&includeSeries=true&includeEpisode=true');
        $baseUrl = rtrim($sonarr['url'], '/');
    ...
    echo json_encode(array_slice($notifs, 0, 30));
    exit;
}
