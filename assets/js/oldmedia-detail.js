...
// Carte dépliable dédiée aux pistes audio (pas de section Vidéo inutile, titre basé sur le nom de fichier)
function buildAudioFileDetailsCard(file, uid, deleteArgs) {
    ...
    ${_fdRow('Canaux', file.audioChannels)}
    ${_fdRow('Débit', file.audioBitRate)}
    ${_fdRow('Échantillonnage', file.sampleRate && file.sampleRate !== '?' ? file.sampleRate + ' Hz' : file.sampleRate)}
    </div>
    <div style="margin-top:14px;">
    <div style="font-size:10px; font-weight:bold; color:var(--muted); text-transform:uppercase; letter-spacing:0.5px; margin-bottom:6px;">${'Autre'}</div>
    ${_fdRow('Chemin', file.path)}
...
async function openEditMediaModal(id, type) {
    ...
    // Définition intelligente du driver pour Lidarr
    ...
    // Gestion du titre de la fenêtre selon la plateforme
    let titleTranslation = '';
    if (type === 'movie') titleTranslation = t('modal_edit_radarr') || "Éditer le film";
    else if (type === 'artist') titleTranslation = "Éditer l'artiste";
    ...
    document.getElementById('btn-save-edit').onclick = async function() {
        ...
        if (r.ok) {
            ...
            // Redirection intelligente en fonction du type
            if (type === 'movie') openMovieDetail(id);
...
// =======================================================================
// === LIDARR : APERÇU RECHERCHE (MUSICBRAINZ)
// =======================================================================
async function openMbArtistDetail(mbId) {
    ...
    const r = await api('mb_artist_detail&mbId=' + encodeURIComponent(mbId), {}, 'GET');
    if (r.error) { content.innerHTML = `<p style="color:var(--accent3); padding:20px;">${esc(r.error)}</p>`; return; }
    ...
    <div class="action-buttons-desktop" style="gap:10px; padding:20px; overflow-x:auto; scrollbar-width:none; border-bottom:1px solid var(--border); margin-bottom:20px;">
    <button id="btn-add-mb" style="background:var(--lidarr); border:none; color:#000; padding:8px 16px; border-radius:8px; font-size:13px; font-weight:800; white-space:nowrap; cursor:pointer; flex-shrink:0; display:flex; gap:6px; align-items:center; box-shadow:0 4px 10px rgba(128,195,66,0.3);" onclick="promptAddMedia('artist', '${esc(mbId)}', '${safeTitle}', this, 'mb')">＋ Ajouter l'artiste</button>
    ...
    <div class="action-buttons-mobile" style="justify-content:center; gap:20px; padding:15px 20px; border-bottom:1px solid var(--border); margin-bottom:20px;">
    <button style="background:var(--lidarr); border:none; color:#000; width:48px; height:48px; border-radius:50%; font-size:20px; cursor:pointer; display:flex; align-items:center; justify-content:center; box-shadow:0 4px 10px rgba(128,195,66,0.3);" onclick="promptAddMedia('artist', '${esc(mbId)}', '${safeTitle}', this, 'mb')">＋</button>
...
// =======================================================================
// === LIDARR : OUVERTURE D'UN ALBUM (Design Fluide type Spotify/nzb360)
// =======================================================================
window.openAlbumView = function(albId, albTitle, albPoster, artistId) {
    ...
    // Design immersif : Fond flouté FIXE, dégradé, pochette, Menu flottant remonté
    const backBtn = `
    <div style="position: relative; min-height: 100vh; background: var(--bg2);">

    <div style="position: fixed; top: -40px; left: -40px; right: -40px; bottom: -40px; background-image: url('${albPoster}'); background-size: cover; background-position: center; filter: blur(40px) brightness(0.6); z-index: 0; pointer-events: none;"></div>

    <div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: linear-gradient(to bottom, rgba(19,22,30,0.2) 0%, var(--bg2) 60%); z-index: 1; pointer-events: none;"></div>

    <div style="position: relative; z-index: 10; width: 100%; box-sizing: border-box;">

    <div style="position: sticky; top: 0; padding: 20px; display: flex; justify-content: space-between; align-items: center; z-index: 20;">
    <button onclick="closeSeasonView()" style="background: transparent; border: none; color: #fff; cursor: pointer; font-size: 26px; display: flex; align-items: center; justify-content: center; opacity: 0.8; transition: opacity 0.2s;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.8'">
    ←
    </button>
    <button onclick="refreshArtist(${artistId}, this)" style="background: transparent; border: none; color: #fff; cursor: pointer; font-size: 24px; opacity: 0.8; transition: opacity 0.2s;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.8'" title="Actualiser">
    ↻
    </button>
    </div>

    <div style="text-align: center; padding: 0 20px 30px 20px;">
    <img src="${albPoster}" style="width: 220px; height: 220px; object-fit: cover; border-radius: 12px; box-shadow: 0 15px 35px rgba(0,0,0,0.6); margin-bottom: 25px;">
    <h2 style="font-size: clamp(24px, 5vw, 32px); font-weight: 800; color: #fff; margin: 0; line-height: 1.2; text-shadow: 0 2px 10px rgba(0,0,0,0.5);">${esc(albTitle)}</h2>
    </div>

    <!-- Ajout de marge en bas (160px) pour ne pas cacher la dernière musique sous le bouton flottant -->
    <div style="max-width: 800px; margin: 0 auto; padding: 0 15px 160px 15px;">
    ${tracksContent}
    </div>
    </div>

    <!-- Bouton flottant remonté (bottom: 90px) pour passer au-dessus de la barre de navigation mobile -->
    <div style="position: fixed; bottom: 90px; right: 20px; z-index: 100; display: flex; flex-direction: column; align-items: flex-end; gap: 12px;">

    <div id="album-search-menu-${albId}" style="display: none; flex-direction: column; gap: 8px; background: var(--bg3); padding: 12px; border-radius: 16px; border: 1px solid var(--border); box-shadow: 0 10px 30px rgba(0,0,0,0.6);">
    <button onclick="document.getElementById('album-search-menu-${albId}').style.display='none'; albumSearchAuto(${albId}, this)" style="background: var(--bg2); border: 1px solid var(--border); color: var(--text); padding: 12px 20px; border-radius: 10px; cursor: pointer; font-weight: 700; font-size: 14px; display: flex; align-items: center; gap: 10px; transition: background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='var(--bg2)'">
    <span style="font-size: 18px;">🤖</span> Recherche automatique
    </button>
    <button onclick="document.getElementById('album-search-menu-${albId}').style.display='none'; openAlbumReleases(${albId}, '${esc(albTitle).replace(/'/g, "\\'")}', ${artistId})" style="background: var(--bg2); border: 1px solid var(--border); color: var(--text); padding: 12px 20px; border-radius: 10px; cursor: pointer; font-weight: 700; font-size: 14px; display: flex; align-items: center; gap: 10px; transition: background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='var(--bg2)'">
    <span style="font-size: 18px;">👤</span> Releases manuelles
    </button>
    </div>

    <button onclick="const m=document.getElementById('album-search-menu-${albId}'); m.style.display=m.style.display==='none'?'flex':'none';" style="width: 56px; height: 56px; background: var(--lidarr); color: #000; border: none; border-radius: 50%; font-size: 22px; cursor: pointer; box-shadow: 0 6px 20px rgba(128,195,66,0.4); display: flex; align-items: center; justify-content: center; transition: transform 0.2s; flex-shrink: 0;" onmouseover="this.style.transform='scale(1.08)'" onmouseout="this.style.transform='scale(1)'" title="${t('detail_auto_search')}">
    🔍
    </button>
    </div>
    </div>
    ...
    setTimeout(() => {
        window.scrollTo(0, 0); // S'assure de remonter tout en haut de la page en l'ouvrant
        artistContent.innerHTML = backBtn;
        artistContent.style.transform = 'translateX(-30px)';
        setTimeout(() => {
...
// =======================================================================
// === LIDARR : FICHE DÉTAIL ARTISTE BIBLIOTHÈQUE
// =======================================================================
async function openArtistDetail(id) {
    ...
    // Suppression du CSS inline pour utiliser la classe CSS responsive .albums-grid
    let albumsHtml = '<div class="albums-grid">';
    ...
    (r.albums || []).forEach(alb => {
        ...
        (alb.tracks || []).sort((a, b) => (a.track || 0) - (b.track || 0)).forEach(tr => {
            ...
            if (tr.hasFile) {
                ...
                statusLabel = `<span style="background: rgba(255, 255, 255, 0.1); color: var(--muted); padding: 2px 6px; border-radius: 4px; font-size: 10px; font-weight: 800; text-transform: uppercase;">Manquant</span>`;
            }
        ...
        <span style="cursor:pointer; display:flex; align-items:center;" onclick="toggleAlbumMonitor(${alb.id}, ${!alb.monitored}, this)" title="Surveiller">${alb.monitored ? ICON_MONITORED : ICON_UNMONITORED}</span>
        <button style="background:none; border:none; color:var(--muted); cursor:pointer; display:flex; align-items:center; transition:color 0.2s; padding:0;" onmouseover="this.style.color='var(--text)'" onmouseout="this.style.color='var(--muted)'" onclick="albumSearchAuto(${alb.id}, this)" title="Recherche automatique">🔍</button>
        <button style="background:none; border:none; color:var(--muted); cursor:pointer; display:flex; align-items:center; transition:color 0.2s; padding:0;" onmouseover="this.style.color='var(--text)'" onmouseout="this.style.color='var(--muted)'" onclick="openAlbumReleases(${alb.id}, '${albTitleSafe}', ${r.id})" title="Releases manuelles">👤</button>
        ${albumFileIds.length > 0 ? `<button style="background:none; border:none; color:var(--accent3); cursor:pointer; display:flex; align-items:center; opacity:0.8; transition:opacity 0.2s; padding:0;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.8'" onclick="deleteAlbumFiles([${albumFileIds.join(',')}], ${r.id}, '${albTitleSafe}')" title="Supprimer">🗑️</button>` : ''}
        </div>
        ...
        <button class="sheet-btn" onclick="closeMobileAlbumMenu(${alb.id}); albumSearchAuto(${alb.id}, this);" style="background: var(--bg3); border: 1px solid var(--border); color: var(--text); padding: 18px; border-radius: 14px; font-size: 16px; font-weight: 500; display: flex; align-items: center; gap: 15px; cursor: pointer; text-align: left;"><span style="font-size: 20px;">🔍</span> Recherche auto</button>
        <button class="sheet-btn" onclick="closeMobileAlbumMenu(${alb.id}); openAlbumReleases(${alb.id}, '${albTitleSafe}', ${r.id});" style="background: var(--bg3); border: 1px solid var(--border); color: var(--text); padding: 18px; border-radius: 14px; font-size: 16px; font-weight: 500; display: flex; align-items: center; gap: 15px; cursor: pointer; text-align: left;"><span style="font-size: 20px;">👤</span> Sorties manuelles</button>
        ${albumFileIds.length > 0 ? `<button class="sheet-btn danger" onclick="closeMobileAlbumMenu(${alb.id}); deleteAlbumFiles([${albumFileIds.join(',')}], ${r.id}, '${albTitleSafe}');" style="background: rgba(255, 93, 143, 0.05); border: 1px solid rgba(255, 93, 143, 0.3); color: var(--accent3); padding: 18px; border-radius: 14px; font-size: 16px; font-weight: 500; display: flex; align-items: center; gap: 15px; cursor: pointer; text-align: left;"><span style="font-size: 20px;">🗑️</span> Supprimer les fichiers</button>` : ''}
        </div>
        </div>`;
    });
    albumsHtml += '</div>';
    ...
    /* Grille responsive pour les albums */
    .albums-grid { display: grid; padding: 0 20px 40px 20px; }

    /* Version PC : Affiches grandes et boutons apparents */
    ...
    /* Version Mobile : Affiches réduites à 95px pour faire 3 colonnes exactes */
    @media (max-width: 768px) {
        .action-buttons-mobile { display: flex !important; }
        .action-buttons-desktop { display: none !important; }
    ...
    <div style="position:relative; width:100%; min-height:100vh; background:var(--bg2); display:flex; justify-content:center;">

    <!-- FOND FLOUTÉ IMMERSIF (même traitement que la vue album), limité en largeur sur grand écran -->
    <div style="position: fixed; top: 0; left: 50%; transform: translateX(-50%); width: min(100%, 1100px); bottom: 0; background-image: url('${fanartUrl}'); background-size: cover; background-position: center; filter: brightness(0.55); z-index: 0; pointer-events: none;"></div>
    <div style="position: fixed; top: 0; left: 50%; transform: translateX(-50%); width: min(100%, 1100px); bottom: 0; background: linear-gradient(to bottom, rgba(19,22,30,0.2) 0%, var(--bg2) 60%); z-index: 1; pointer-events: none;"></div>

    <div style="position: relative; z-index: 10; width: 100%; max-width: 1100px; box-sizing: border-box;">

    <!-- BARRE COLLANTE : RETOUR -->
    <div style="position: sticky; top: 0; padding: 20px; display: flex; justify-content: space-between; align-items: center; z-index: 20;">
    <button onclick="closeSerieDetail()" style="background: transparent; border: none; color: #fff; cursor: pointer; font-size: 26px; display: flex; align-items: center; justify-content: center; opacity: 0.8; transition: opacity 0.2s;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.8'">←</button>
    <button onclick="refreshArtist(${r.id}, this)" style="background: transparent; border: none; color: #fff; cursor: pointer; font-size: 24px; opacity: 0.8; transition: opacity 0.2s;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.8'" title="${t('detail_refresh')}">↻</button>
    </div>

    <!-- PORTRAIT + TITRE CENTRÉS -->
    <div style="text-align:center; padding:0 20px 25px 20px;">
    ${posterUrl ? `<img src="${posterUrl}" style="width:150px; height:150px; border-radius:50%; box-shadow:0 15px 35px rgba(0,0,0,0.6); object-fit:cover; margin-bottom:20px; border:3px solid rgba(255,255,255,0.1);">` : `<div style="width:150px; height:150px; background:var(--bg3); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:50px; box-shadow:0 15px 35px rgba(0,0,0,0.6); border:3px solid rgba(255,255,255,0.1); margin:0 auto 20px auto;">🎵</div>`}
    <div id="movie-status-badge" style="display:inline-block; font-size:10px; font-weight:bold; padding:3px 8px; border-radius:6px; background:var(--bg3); border:1px solid var(--border); color:${statusColor}; margin-bottom:10px;">${r.pct}% • ${statusText}</div>
    <h2 style="font-size:clamp(24px, 5vw, 32px); font-weight:800; line-height:1.2; margin:0 0 10px 0; color:#fff; text-shadow:0 2px 10px rgba(0,0,0,0.5);">${esc(r.title)}</h2>
    <div style="font-size:13px; color:rgba(255,255,255,0.8); display:flex; gap:12px; flex-wrap:wrap; align-items:center; justify-content:center;">
    ${r.rating ? `<span style="background:rgba(255,255,255,0.1); padding:2px 8px; border-radius:6px; color:#fff;">⭐ ${r.rating}</span>` : ''}
    ...
    <!-- BOUTONS D'ACTION ARTISTE (Design Série/Film) -->
    <div class="action-buttons-desktop" style="gap:10px; padding:20px; overflow-x:auto; scrollbar-width:none; border-bottom:1px solid var(--border); margin-bottom:20px; justify-content:center;">
    <button style="background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.15); color:#fff; padding:8px 16px; border-radius:8px; font-size:13px; font-weight:600; white-space:nowrap; cursor:pointer; flex-shrink:0; display:flex; gap:6px; align-items:center; transition:background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.15)'" onmouseout="this.style.background='rgba(255,255,255,0.08)'" onclick="artistSearchAuto(${r.id}, this)">🔍 Recherche automatique</button>
    <button style="background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.15); color:#fff; padding:8px 16px; border-radius:8px; font-size:13px; font-weight:600; white-space:nowrap; cursor:pointer; flex-shrink:0; display:flex; gap:6px; align-items:center; transition:background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.15)'" onmouseout="this.style.background='rgba(255,255,255,0.08)'" onclick="openEditMediaModal(${r.id}, 'artist')">⚙️ Éditer l'artiste</button>
    <button style="background:rgba(255,93,143,0.15); border:1px solid rgba(255,93,143,0.3); color:#ff9db3; padding:8px 16px; border-radius:8px; font-size:13px; font-weight:600; white-space:nowrap; cursor:pointer; flex-shrink:0; display:flex; gap:6px; align-items:center; transition:background 0.2s;" onmouseover="this.style.background='rgba(255,93,143,0.25)'" onmouseout="this.style.background='rgba(255,93,143,0.15)'" onclick="deleteArtist(${r.id}, '${safeTitle}')">🗑️ Supprimer</button>
    </div>

    <div class="action-buttons-mobile" style="justify-content:center; gap:15px; padding:15px 20px; border-bottom:1px solid var(--border); margin-bottom:20px; flex-wrap:wrap;">
    <button style="background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.15); border-radius:50%; color:#fff; width:44px; height:44px; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:18px; box-shadow:0 4px 10px rgba(0,0,0,0.2);" onclick="artistSearchAuto(${r.id}, this)" title="Recherche Auto">🔍</button>
    <button style="background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.15); border-radius:50%; color:#fff; width:44px; height:44px; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:18px; box-shadow:0 4px 10px rgba(0,0,0,0.2);" onclick="openEditMediaModal(${r.id}, 'artist')" title="Éditer">⚙️</button>
    <button style="background:rgba(255,93,143,0.1); border:1px solid rgba(255,93,143,0.25); border-radius:50%; color:#ff9db3; width:44px; height:44px; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:18px; box-shadow:0 4px 10px rgba(0,0,0,0.2);" onclick="deleteArtist(${r.id}, '${safeTitle}')" title="Supprimer">🗑️</button>
    </div>

    <!-- STATS BAR -->
    <div style="padding:0 20px 15px 20px; display:flex; justify-content:space-between; align-items:flex-end; max-width:800px; margin:0 auto; box-sizing:border-box;">
    <div>
    <div style="font-size:12px; color:rgba(255,255,255,0.7); margin-bottom:15px;">
    <span>${genres}</span>
    </div>
    <h3 style="margin:0; font-size:22px; color:#fff; font-weight:800;">${(r.albums||[]).length} Albums</h3>
    </div>
    <div style="font-size:12px; font-family:var(--mono); color:var(--lidarr); font-weight:bold; background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.15); padding:8px 12px; border-radius:8px;">
...
// =======================================================================
// === LIDARR : FONCTIONS D'ACTIONS GLOBALES POUR LES ARTISTES ET ALBUMS
// =======================================================================
...
function deleteArtist(id, title) {
    showConfirmModal(
        (t('confirm_delete_title') || 'Supprimer') + ' — ' + title,
                     (t('confirm_delete_msg_media') || 'Voulez-vous vraiment supprimer {title} ?').replace('{title}', title),
                     async () => {
                         const r = await api('delete_artist', { artistId: id, deleteFiles: 'false' });
                         if (r.ok) {
                             notify((t('deleted_ok') || 'Supprimé avec succès').replace('{title}', title), 'ok');
                             closeSerieDetail();
                             if (typeof loadArtists === 'function') loadArtists();
                         } else {
                             notify(r.error || t('delete_error') || 'Erreur lors de la suppression', 'err');
                         }
                     }
    );
}
...
async function artistSearchAuto(artistId, btn) {
    ...
    if (r.ok) { btn.textContent = '✓'; notify('Recherche de l\'artiste lancée', 'ok'); }
    else { btn.disabled = false; btn.textContent = '🔍 Recherche automatique'; notify(r.error || 'Erreur', 'err'); }
}

async function albumSearchAuto(albumId, btn) {
    btn.disabled = true; btn.textContent = '⏳';
    const r = await api('album_search_auto', { albumId });
    if (r.ok) { btn.textContent = '✓'; notify(t('search_started') || 'Recherche lancée', 'ok'); }
    else { btn.disabled = false; btn.textContent = '🔍'; notify(r.error || t('notif_error') || 'Erreur', 'err'); }
}
...
async function openAlbumReleases(albumId, label, artistId) {
    ...
    renderReleasesTable(r.releases || [], 'album', artistId);
}

function deleteAlbumFiles(fileIds, artistId, albumTitle) {
    showConfirmModal(
        (t('detail_delete') || 'Supprimer') + ' — ' + albumTitle,
                     t('confirm_delete_msg') || 'Voulez-vous vraiment supprimer ces fichiers ?',
                     async () => {
                         ...
                         if (successCount > 0) {
                             notify((t('deleted_ok') || 'Suppression réussie').replace('{title}', albumTitle), 'ok');
                             openArtistDetail(artistId); // Recharge l'artiste pour mettre à jour l'interface
                         } else {
                             notify(t('delete_error') || 'Erreur lors de la suppression', 'err');
                         }
                     }
    );
}
